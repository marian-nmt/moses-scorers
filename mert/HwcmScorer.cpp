...
// HWCM score (Liu and Gildea, 2005). Implements F1 instead of precision for better modelling of
// hypothesis length. assumes dependency trees on target side (generated by
// scripts/training/wrappers/conll2mosesxml.py ; use with option --brackets for reference). reads
// reference trees from separate file {REFERENCE_FILE}.trees to support mix of string-based and
// tree-based metrics.
...
HwcmScorer::HwcmScorer(const string& config) : StatisticsBasedScorer("HWCM", config) {}
...
void HwcmScorer::setReferenceFiles(const vector<string>& referenceFiles) {
  ...
  while(getline(in, line)) {
    ...
    for(size_t i = 0; i < kHwcmOrder; i++) {
      for(map<string, int>::const_iterator it = m_ref_hwc.back()[i].begin();
          it != m_ref_hwc.back()[i].end();
          it++) {
        totals[i] += it->second;
      }
    }
...
void HwcmScorer::extractHeadWordChain(TreePointer tree,
                                      vector<string>& history,
                                      vector<map<string, int> >& hwc) {
  if(tree->GetLength() > 0) {
    string head = getHead(tree);

    if(head.empty()) {
      for(std::vector<TreePointer>::const_iterator it = tree->GetChildren().begin();
...
string HwcmScorer::getHead(TreePointer tree) {
  // assumption (only true for dependency parse: each constituent has a preterminal label, and
  // corresponding terminal is head) if constituent has multiple preterminals, first one is picked;
  // if it has no preterminals, empty string is returned
  for(std::vector<TreePointer>::const_iterator it = tree->GetChildren().begin();
      it != tree->GetChildren().end();
      ++it) {
