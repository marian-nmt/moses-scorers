...
void SemposScorer::prepareStats(size_t sid, const string& text, ScoreStats& entry) {
  ...
  if(m_ref_sentences.size() == 1) {
    ...
    for(size_t rid = 0; rid < m_ref_sentences.size(); ++rid) {
      const vector<ScoreStatsType>& tmp
          = m_ovr->prepareStats(encodedCandSentence, m_ref_sentences[rid][sid]);
...
void SemposScorer::splitSentence(const string& sentence, str_sentence_t& splitSentence) {
  ...
  for(vector<string>::iterator it = tokens.begin(); it != tokens.end(); ++it) {
    ...
    split(*it, '|', factors);
    if(factors.size() != 2)
      throw runtime_error("Sempos scorer accepts two factors (item|class)");
...
void SemposScorer::loadWeights(const string& weightsfile) {
  ...
  if(myfile.is_open()) {
    while(myfile.good()) {
      ...
      split(line, '\t', fields);
      if(fields.size() != 2)
        throw std::runtime_error("Bad format of a row in weights file.");
