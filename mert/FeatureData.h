...
public:
  ...
  inline FeatureStats& get(std::size_t i, std::size_t j) { return m_array.at(i).get(j); }
  ...
  bool existsFeatureNames() const { return (m_index_to_feature_name.size() > 0) ? true : false; }
  ...
  std::string getFeatureName(std::size_t idx) const {
    ...
    if(it == m_index_to_feature_name.end()) {
      throw std::runtime_error("Error: specified id is unknown: "
                               + boost::lexical_cast<std::string>(idx));
    } else {
      return it->second;
    }
  ...
  std::size_t getFeatureIndex(const std::string& name) const {
    ...
    if(it == m_feature_name_to_index.end()) {
      ...
      for(std::map<std::string, std::size_t>::const_iterator cit = m_feature_name_to_index.begin();
          cit != m_feature_name_to_index.end();
          cit++) {
        msg += cit->first;
        msg += ", ";
      }
