...
uint64_t Timer::get_elapsed_cpu_time_microseconds() const {
  ...
  return (e.user_time - m_start_time.user_time) + (e.sys_time - m_start_time.sys_time);
}
...
void Timer::check(const char* msg) {
  ...
  if(m_is_running) {
    TRACE_ERR("[Wall " << get_elapsed_wall_time() << " CPU " << get_elapsed_cpu_time()
                       << "] seconds.\n");
  } else {
...
std::string Timer::ToString() const {
  ...
  std::stringstream ss;
  ss << "wall " << wall << " sec. user " << utime << " sec. sys " << stime << " sec. total "
     << utime + stime << " sec.";
  res.append(ss.str());
