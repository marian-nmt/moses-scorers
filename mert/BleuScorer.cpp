...
BleuScorer::BleuScorer(const string& config)
    : StatisticsBasedScorer("BLEU", config), m_ref_length_type(CLOSEST) {
...
size_t BleuScorer::CountNgrams(const string& line,
                               NgramCounts& counts,
                               unsigned int n,
...
bool BleuScorer::OpenReferenceStream(istream* is, size_t file_id) {
  ...
  while(getline(*is, line)) {
    // TODO: rather than loading the whole reference corpus into memory, can we stream it line by
    // line?
    //  (loading the whole reference corpus can take gigabytes of RAM if done with millions of
    //  sentences)
    ...
    UTIL_THROW_IF2(m_references.size() <= sid,
                   "Reference " << file_id << "has too many sentences.");
...
void BleuScorer::CalcBleuStats(const Reference& ref,
...
int BleuScorer::CalcReferenceLength(const Reference& ref, std::size_t length) const {
  switch(m_ref_length_type) {
    case AVERAGE: return ref.CalcAverage(); break;
    case CLOSEST: return ref.CalcClosest(length); break;
    case SHORTEST: return ref.CalcShortest(); break;
    default: UTIL_THROW2("Unknown reference types");
  }
}

void BleuScorer::DumpCounts(ostream* os, const NgramCounts& counts) const {
  for(NgramCounts::const_iterator it = counts.begin(); it != counts.end(); ++it) {
    *os << "(";
...
float smoothedSentenceBleu(const std::vector<float>& stats, float smoothing, bool smoothBP) {
  ...
  logbleu /= kBleuNgramOrder;
  const float reflength = stats[(kBleuNgramOrder * 2)] + (smoothBP ? smoothing : 0.0f);
