...
TerScorer::TerScorer(const string& config) : StatisticsBasedScorer("TER", config), kLENGTH(2) {}
...
void TerScorer::prepareStats(size_t sid, const string& text, ScoreStats& entry) {
  ...
  stats << result.numEdits * 100.0 << " " << result.averageWords * 100.0 << " "
        << result.scoreAv() * 100.0 << " ";
