...
MeteorScorer::MeteorScorer(const string& config) : StatisticsBasedScorer("METEOR", config) {
  ...
  if(meteor_jar == "") {
    throw runtime_error(
        "Meteor jar required, see MeteorScorer.h for full list of options: --scconfig "
        "jar:/path/to/meteor-1.4.jar");
  }
  int pipe_status;
  int pipefds_input[2];
  ...
  if(pid == pid_t(0)) {
    ...
    meteor_cmd << "java -Xmx1G -jar " << meteor_jar << " - - -stdio -lower -t " << meteor_task
               << " -l " << meteor_lang;
    if(meteor_m != "") {
