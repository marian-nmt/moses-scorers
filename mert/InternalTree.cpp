...
InternalTree::InternalTree(const std::string &line, const bool terminal) : m_isTerminal(terminal) {
...
std::string InternalTree::GetString(bool start) const {
  ...
  ret += m_value;
  for(std::vector<TreePointer>::const_iterator it = m_children.begin(); it != m_children.end();
      ++it) {
...
void InternalTree::Combine(const std::vector<TreePointer> &previous) {
  ...
  for(std::vector<TreePointer>::const_iterator it_prev = previous.begin();
      it_prev != previous.end();
      ++it_prev) {
