...
int GzFileBuf::underflow() {
   ...
   */
  std::memmove(m_buf + (sizeof(int) - num_put_back), gptr() - num_put_back, num_put_back);

  // read new characters
  const int num = gzread(m_gz_file, m_buf + sizeof(int), kBufSize - sizeof(int));
  if(num <= 0) {
    // ERROR or EOF
...
std::streampos GzFileBuf::seekpos(std::streampos sp, std::ios_base::openmode which) {
  throw;
}

std::streamsize GzFileBuf::xsgetn(char* s, std::streamsize num) {
  return static_cast<std::streamsize>(gzread(m_gz_file, s, num));
}

std::streamsize GzFileBuf::xsputn(const char* s, std::streamsize num) {
  throw;
}
