...
SentenceLevelScorer::SentenceLevelScorer(const string& name, const string& config)
    : Scorer(name, config), m_regularisationStrategy(REG_NONE), m_regularisationWindow(0) {
  Init();
}
...
void SentenceLevelScorer::score(const candidates_t& candidates,
                                const diffs_t& diffs,
                                statscores_t& scores) {
  ...
  for(size_t i = 0; i < diffs.size(); ++i) {
    for(size_t j = 0; j < diffs[i].size(); ++j) {
      ...
      for(size_t k = 0; k < totals.size(); ++k) {
        const float diff
            = m_score_data->get(sid, nid).get(k) - m_score_data->get(sid, last_nid).get(k);
        // cout << "diff = " << diff << endl;
