...
hashMapInfos terCalc::createConcordMots(vector<string>& hyp, vector<string>& ref) {
  ...
  for(int start = 0; start < (int)ref.size(); start++) {
    if(cor[start]) {
      for(int end = start;
          ((end < (int)ref.size()) && (end - start <= TAILLE_PERMUT_MAX) && (cor[end]));
          end++) {
...
terAlignment terCalc::minimizeDistanceEdition(vector<string>& hyp,
                                              vector<string>& ref,
  ...
  for(j = 0; j <= hyp_size; j++) {
    ...
    for(i = first_good; i <= ref_size; i++) {
      ...
      if((i < ref_size) && (j < hyp_size)) {
        if((int)refSpans.size() == 0 || (int)hypSpans.size() == 0
           || trouverIntersection(refSpans.at(i), curHypSpans.at(j))) {
  ...
  while((i > 0) || (j > 0)) {
    ...
    if(P->at(i).at(j) == 'A') {
      ...
      cerr << "ERROR : terCalc::minimizeDistanceEdition : Invalid path : " << P->at(i).at(j)
           << endl;
      exit(-1);
    }
  }
  ...
  if(PRINT_DEBUG) {
    cerr << "BEGIN DEBUG : terCalc::minimizeDistanceEdition : to_return :" << endl
         << to_return.toString() << endl
         << "END DEBUG" << endl;
  }
  return to_return;
}
void terCalc::minimizeDistanceEdition(vector<string>& hyp,
                                      vector<string>& ref,
                                      vector<vecInt>& curHypSpans,
                                      terAlignment* to_return) {
  double current_best = infinite;
...
terAlignment terCalc::TER(vector<string>& hyp, vector<string>& ref) {
  ...
  if(PRINT_DEBUG) {
    cerr << "BEGIN DEBUG : terCalc::TER : cur_align :" << endl
         << cur_align.toString() << endl
         << "END DEBUG" << endl;
  }
  while(true) {
    returns = findBestShift(cur, hyp, ref, rloc, cur_align);
    //             cerr << "****************************************************************** " <<
    //             returns->getEmpty() << endl;
    if(returns->getEmpty()) {
      break;
    }
  ...
  if(PRINT_DEBUG) {
    cerr << "BEGIN DEBUG : terCalc::TER : Final to return :" << endl
         << cur_align.toString() << endl
         << "END DEBUG" << endl;
  }
  terAlignment to_return;
...
bestShiftStruct* terCalc::findBestShift(vector<string>& cur,
                                        vector<string>& hyp,
                                        vector<string>& ref,
                                        hashMapInfos& rloc,
  ...
  if(PRINT_DEBUG) {
    ...
    cerr << "herr   : " << vectorToString(herr, "\t", (int)hyp.size()) << " | " << (int)hyp.size()
         << endl;
    cerr << "rerr   : " << vectorToString(rerr, "\t", (int)ref.size()) << " | " << (int)ref.size()
         << endl;
    cerr << "ralign : " << vectorToString(ralign, "\t", (int)ref.size()) << " | " << (int)ref.size()
         << endl;
    cerr << "END DEBUG " << endl;
  }
  ...
  for(i = (int)poss_shifts->size() - 1; i >= 0; i--) {
    if(PRINT_DEBUG) {
      cerr << "BEGIN DEBUG : terCalc::findBestShift :" << endl;
      cerr << "Considering shift of length " << i << " (" << (poss_shifts->at(i)).size() << ")"
           << endl;
      cerr << "END DEBUG " << endl;
    }
    ...
    if((curfix > maxfix) || ((cur_best_shift_cost != 0) && (curfix == maxfix))) {
      ...
      for(s = 0; s < (int)(poss_shifts->at(i)).size(); s++) {
        ...
        if((curfix > maxfix) || ((cur_best_shift_cost != 0) && (curfix == maxfix))) {
          ...
          if(PRINT_DEBUG) {
            cerr << "BEGIN DEBUG : terCalc::findBestShift :" << endl;
            cerr << "Gain for " << curshift->toString() << " is " << gain << ". (result: ["
                 << curalign->join(" ", shiftarr) << "]" << endl;
            cerr << "Details of gains : gain = ( cur_best_align->numEdits + cur_best_shift_cost ) "
                    "- ( curalign->numEdits + curshift->cost )"
                 << endl;
            cerr << "Details of gains : gain = (" << cur_best_align->numEdits << "+"
                 << cur_best_shift_cost << ") - (" << curalign->numEdits << "+" << curshift->cost
                 << ")" << endl;
...
void terCalc::calculateTerAlignment(terAlignment& align,
                                    vector<bool>* herr,
                                    vector<bool>* rerr,
                                    vector<int>* ralign) {
  int hpos = -1;
  ...
  for(int i = 0; i < (int)align.alignment.size(); i++) {
    ...
    if(sym == 'A') {
      ...
      cerr << "ERROR : terCalc::calculateTerAlignment : Invalid mini align sequence " << sym
           << " at pos " << i << endl;
      exit(-1);
    }
  }
...
vector<vecTerShift>* terCalc::calculerPermutations(vector<string>& hyp,
                                                   vector<string>& ref,
                                                   hashMapInfos& rloc,
  ...
  for(start = 0; start < (int)hyp.size(); start++) {
    ...
    while(mti != mtiVec.end() && (!ok)) {
      moveto = (*mti);
      mti++;
      if((start != ralign->at(moveto)) && ((ralign->at(moveto) - start) <= DIST_MAX_PERMUT)
         && ((start - ralign->at(moveto) - 1) <= DIST_MAX_PERMUT)) {
        ok = true;
      }
    }
    if(!ok) {
    ...
    for(end = start; (ok && (end < (int)hyp.size()) && (end < start + TAILLE_PERMUT_MAX)); end++) {
      ...
      // 		cerr << "CANDIDATE " << ( string ) vectorToString ( cand )  <<" PLACED : " << ( string )
      // vectorToString ( movetoitVec," ") << endl;
      vector<int>::iterator movetoit;
      movetoit = movetoitVec.begin();
      while(movetoit != movetoitVec.end()) {
        moveto = (*movetoit);
        movetoit++;
        if(!((ralign->at(moveto) != start)
             && ((ralign->at(moveto) < start) || (ralign->at(moveto) > end))
             && ((ralign->at(moveto) - start) <= DIST_MAX_PERMUT)
             && ((start - ralign->at(moveto)) <= DIST_MAX_PERMUT))) {
          continue;
        }
        ok = true;
        ...
        */
        ...
        for(int roff = -1; roff <= (end - start); roff++) {
          ...
          if((roff == -1) && (moveto == 0)) {
            if(PRINT_DEBUG) {
              cerr << "BEGIN DEBUG : terCalc::calculerPermutations 01 : " << endl
                   << "Consider making " << start << "..." << end << " ("
                   << vectorToString(cand, " ") << ") moveto: " << moveto << " roff: " << roff
                   << " ralign[mt+roff]: -1" << endl
                   << "END DEBUG" << endl;
            }
            ...
            topush->newloc = -1;
            topushNull = false;
          } else if((start != ralign->at(moveto + roff))
                    && ((roff == 0) || (ralign->at(moveto + roff) != ralign->at(moveto)))) {
            int newloc = ralign->at(moveto + roff);
            if(PRINT_DEBUG) {
              cerr << "BEGIN DEBUG : terCalc::calculerPermutations 02 : " << endl
                   << "Consider making " << start << "..." << end << " ("
                   << vectorToString(cand, " ") << ") moveto: " << moveto << " roff: " << roff
                   << " ralign[mt+roff]: " << newloc << endl
                   << "END DEBUG" << endl;
            }
...
alignmentStruct terCalc::permuter(vector<string>& words, int start, int end, int newloc) {
  ...
  if(PRINT_DEBUG) {
    if((int)hypSpans.size() > 0) {
      cerr << "BEGIN DEBUG : terCalc::permuter :" << endl
           << "word length: " << (int)words.size() << " span length: " << (int)hypSpans.size()
           << endl;
    } else {
      cerr << "BEGIN DEBUG : terCalc::permuter :" << endl
           << "word length: " << (int)words.size() << " span length: null" << endl;
    }
    cerr << "BEGIN DEBUG : terCalc::permuter :" << endl
         << join(" ", words) << " start: " << start << " end: " << end << " newloc " << newloc
         << endl
         << "END DEBUG " << endl;
  }
  if(newloc >= (int)words.size()) {
    if(PRINT_DEBUG) {
      cerr << "WARNING: Relocation over the size of the hypothesis, replacing at the end of it."
           << endl;
    }
