...
void Scorer::TokenizeAndEncode(const string& line, vector<int>& encoded) const {
  for(util::TokenIter<util::AnyCharacter, true> it(line, util::AnyCharacter(" ")); it; ++it) {
    if(!m_enable_preserve_case) {
      string token = it->as_string();
      for(std::string::iterator sit = token.begin(); sit != token.end(); ++sit) {
        *sit = tolower(*sit);
      }
...
string Scorer::applyFactors(const string& sentence) const {
  ...
  for(size_t i = 0; i < tokens.size(); ++i) {
    ...
    for(size_t j = 0; j < m_factors.size(); ++j) {
      ...
      if(findex < 0 || findex >= fsize)
        throw runtime_error("Factor index is out of range.");
