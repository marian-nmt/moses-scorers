...
void Permutation::set(const string &alignment, const int sourceLength) {
  ...
  for(size_t i = 0; i < tokens.size(); i++) {
    ...
    if(sourcePos > sourceLength) {
      cerr << "Source sentence length:" << sourceLength
           << " is smaller than alignment source position:" << sourcePos << endl;
...
float Permutation::distance(const Permutation &permCompare, const distanceMetric_t &type) const {
  ...
  float brevityPenalty = 1.0
                         - (float)permCompare.getTargetLength()
                               / getTargetLength();  // reflength divided by trans length
