...
void PermutationScorer::prepareStats(size_t sid, const string& text, ScoreStats& entry) {
  ...
  // here we check if the alignments extracted from the nbest are phrase-based or word-based, in
  // which case no conversion is needed
  ...
  if(m_refChoiceStrategy == REFERENCE_CHOICE_AVERAGE) {
    ...
    // cout << "MultRef strategy AVERAGE: total " << total << " mean " << mean << " number " <<
    // m_referencePerms.size() << endl;
    distanceValue = mean;
  ...
  tempStream.precision(
      0);  // decimal precision not needed as score was multiplied per SCORE_MULTFACT
  tempStream << std::fixed << distanceValue
             << " 1";  // use for final normalization over the amount of test sentences
