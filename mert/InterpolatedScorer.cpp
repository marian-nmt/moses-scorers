...
InterpolatedScorer::InterpolatedScorer(const string& name, const string& config)
  ...
  if(wtype.length() == 0) {
    ...
    if(tot != float(1)) {  // TODO: fix this checking in terms of readability.
      for(vector<float>::iterator it = m_scorer_weights.begin(); it != m_scorer_weights.end();
          ++it) {
        *it /= tot;
      }
    }
  ...
  for(ScopedVector<Scorer>::iterator itsc = m_scorers.begin(); itsc != m_scorers.end(); ++itsc) {
    ...
    for(size_t i = 0; i < data->size(); i++) {
      ...
      for(size_t j = 0; j < numNBest; j++) {
        ...
        // cout << " last " << last << " NumScores " << numScoresScorer << "newScorestats " <<
        // newScoreStats << endl;
...
void InterpolatedScorer::score(const candidates_t& candidates,
  ...
  for(ScopedVector<Scorer>::const_iterator itsc = m_scorers.begin(); itsc != m_scorers.end();
    ...
    for(statscores_t::iterator itstatsc = tscores.begin(); itstatsc != tscores.end(); ++itstatsc) {
      ...
      // cout << "Scorer:" << scorerNum <<  " scoreNum:" << inc << " score: " << (*itstatsc) << "
      // weight:" << weight << endl;
      inc++;
    }
    scorerNum++;
...
/** Interpolated scorer gets a vector of sufficient statistics, calls all scorers with corresponding
   statistics, and combines them with weights **/
float InterpolatedScorer::calculateScore(const std::vector<ScoreStatsType>& totals) const {
  ...
  for(ScopedVector<Scorer>::const_iterator itsc = m_scorers.begin(); itsc != m_scorers.end();
      ++itsc) {
    int numScoresScorer = (*itsc)->NumberOfScores();
    std::vector<ScoreStatsType> totals_scorer(totals.begin() + last,
                                              totals.begin() + last + numScoresScorer);
...
void InterpolatedScorer::setFactors(const string& factors) {
  ...
  if(fsplit.size() != m_scorers.size())
    throw runtime_error(
        "Number of factor specifications does not equal number of interpolated scorers.");
...
void InterpolatedScorer::setFilter(const string& filterCommand) {
  ...
  if(csplit.size() != m_scorers.size())
    throw runtime_error(
        "Number of command specifications does not equal number of interpolated scorers.");
