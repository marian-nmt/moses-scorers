...
SparseVector& SparseVector::operator+=(const SparseVector& rhs) {
  for(fvector_t::const_iterator i = rhs.m_fvector.begin(); i != rhs.m_fvector.end(); ++i) {
...
std::vector<std::size_t> SparseVector::feats() const {
  ...
  for(fvector_t::const_iterator iter = m_fvector.begin(); iter != m_fvector.end(); iter++) {
...
std::size_t hash_value(SparseVector const& item) {
  size_t seed = 0;
  for(SparseVector::fvector_t::const_iterator i = item.m_fvector.begin(); i != item.m_fvector.end();
      ++i) {
...
FeatureStats::FeatureStats()
    : m_available_size(kAvailableSize),
      m_entries(0),
      m_array(new FeatureStatsType[m_available_size]) {}

FeatureStats::FeatureStats(const size_t size)
    : m_available_size(size), m_entries(size), m_array(new FeatureStatsType[m_available_size]) {
...
void FeatureStats::loadbin(istream* is) {
  is->read(reinterpret_cast<char*>(m_array), static_cast<streamsize>(GetArraySizeWithBytes()));
}
...
void FeatureStats::savebin(ostream* os) {
  os->write(reinterpret_cast<char*>(m_array), static_cast<streamsize>(GetArraySizeWithBytes()));
}
