...
bool Scan<bool>(const std::string& input) {
  ...
  UTIL_THROW(BoolValueException,
             "Could not interpret " << input
                                    << " as a boolean.  After lowercasing, valid values are yes, "
                                       "y, true, 1, no, n, false, and 0.");
}
  ...
  while(check_dlt) {
    ...
    if(dlt != "") {
      ...
      if(start_type != std::string::npos) {
        ...
        UTIL_THROW(
            util::Exception,
            "ProcessAndStripDLT(std::string &line): Attribute type for dlt tag is mandatory.");
      }
