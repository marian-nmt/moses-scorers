...
#define FEATUREVERBOSE(level, str) \
  FEATUREVERBOSE2(level, "[" << GetScoreProducerDescription() << "] " << str)
#define FEATUREVERBOSE2(level, str)             \
  {                                             \
    IFFEATUREVERBOSE(level) { TRACE_ERR(str); } \
  }
#define IFFEATUREVERBOSE(level)                              \
  if((m_verbosity == std::numeric_limits<std::size_t>::max() \
      && StaticData::Instance().GetVerboseLevel() >= level)  \
     || (m_verbosity != std::numeric_limits<std::size_t>::max() && m_verbosity >= level))
...
inline XmlInputType Scan<XmlInputType>(const std::string& input) {
  ...
  if(input == "exclusive")
    ret = XmlExclusive;
  else if(input == "inclusive")
    ret = XmlInclusive;
  else if(input == "constraint")
    ret = XmlConstraint;
  else if(input == "ignore")
    ret = XmlIgnore;
  else if(input == "pass-through")
    ret = XmlPassThrough;
  else {
...
/** tokenise input string to vector of string. each element has been separated by a character in the
   delimiters argument. The separator can only be 1 character long. The default delimiters are space
   or tab
*/
 ...
 * Not sure whether give zens a medal for being a genius, or shoot him for writing unreadable code.
 * Mabe both...
 */
inline float CalcTranslationScore(const std::vector<float>& probVector,
                                  const std::vector<float>& weightT) {
  UTIL_THROW_IF2(weightT.size() != probVector.size(), "Weight and score vector sizes not the same");
  float rv = 0.0;
  for(float const *sb = &probVector[0], *se = sb + probVector.size(), *wb = &weightT[0]; sb != se;
      ++sb, ++wb)
...
//! definition of ToString() function to go in .cpp file. Can be used for any class that can be
//! piped to a stream
...
std::string PassthroughSGML(std::string& line,
                            const std::string tagName,
                            const std::string& lbrackStr = "<",
                            const std::string& rbrackStr = ">");

/**
 * Returns the first string bounded by the delimiters (default delimiters are " " and "\t")i
 * starting from position first_pos and and stores the starting position of the next string (in
 * first_str)
 */
inline std::string GetFirstString(const std::string& str,
                                  int& first_pos,
                                  const std::string& delimiters = " \t") {
